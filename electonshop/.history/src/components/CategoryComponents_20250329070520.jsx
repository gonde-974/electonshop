        ...
        <div className="relative w-full flex flex-col items-start p-4">
            ...
            {isCategoryVisible && (
                <div className="absolute top-0 left-0 w-full bg-white shadow-lg p-4 rounded-lg z-50">
                    ...
                    <div className="hidden sm:flex flex-wrap justify-start gap-6 mt-4">
                        {category.map((cat, index) => (
                            <Link 
                                key={index} 
                                to={`/category/${cat.id}`} 
                                className="px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg shadow-xl hover:scale-105 transform transition-all duration-300 hover:bg-blue-700 hover:shadow-2xl"
                            >
                                {cat.name}
                            </Link>
                        ))}
