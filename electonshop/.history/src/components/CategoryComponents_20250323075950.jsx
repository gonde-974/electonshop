...
function CategoryComponents() {
    ...
    return (
        <div className="relative p-4">
            ...
            {/* DROP-DOWN МЕНИ */}
            {menuOpen && (
       <div className="absolute mt-2 left-0 w-64 bg-white border border-gray-200 rounded-lg shadow-lg animate-fadeIn">
       <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 p-2">
           {category.map((cat, index) => (
               <li 
                   key={index} 
                   className="rounded-lg transition-all duration-300 bg-gray-50 hover:bg-lightGreenColor hover:text-white shadow-sm hover:shadow-md"
               >
                   <Link 
                       to={`/category/${cat.id}`} 
                       className="block px-4 py-3 text-gray-800 hover:text-white text-center font-medium"
                   >
                       {cat.name}
                   </Link>
               </li>
