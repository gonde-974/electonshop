...
function CountrySelector({product}) {
  const [selectedCountry, setSelectedCountry] = useState(null);
  const [showAlert, setShowAlert] = useState(false);
  const [product,setProduct] = useSelector(state=>state.productStore)
  const productPriceUSD = `${product.price}`; // Base product price in USD
    ...
    <div className="mb-6 relative">
      ...
      {showAlert && selectedCountry && (
        <div className="absolute top-full left-0 mt-4 w-full bg-white border border-gray-300 rounded-lg shadow-md p-4 z-50">
          ...
          <p><strong>Price in USD:</strong> ${productPriceUSD}</p>
