        ...
        <div className="relative w-full flex flex-col items-start p-4">
            ...
            {!menuCategoryOpen && (
                <button 
                    ...
                    className="px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg shadow-md transition-transform hover:scale-105"
                >
                    Category
                </button>
            )}
                    ...
                    <div className="flex flex-wrap gap-2">
                        {category.map((cat, index) => (
                            <Link 
                                key={index} 
                                to={`/category/${cat.id}`} 
                                className="px-4 py-2 bg-gray-100 rounded-lg hover:bg-blue-500 hover:text-white transition-all"
                            >
                                {cat.name}
                            </Link>
                        ))}
