    ...
    <div className="absolute mt-2 left-2 w-auto min-w-[250px] max-w-[98vw] bg-white border border-gray-300 rounded-lg shadow-lg animate-fadeIn">
        <ul className="p-3 flex flex-wrap gap-2 md:flex-nowrap md:flex-row md:overflow-x-auto whitespace-nowrap scrollbar-hide">
            {category.map((cat, index) => (
                <li 
                    key={index} 
                    className="px-4 py-2 bg-white text-gray-800 rounded-lg shadow-md transition-all duration-300 hover:shadow-lg hover:bg-mainBlue hover:text-white hover:scale-105"
                >
                    <Link 
                        to={`/category/${cat.id}`} 
