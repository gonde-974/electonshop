...
function CategoryComponents({ setMenuCategoryOpen }) {
    ...
    const [isHomePageVisible, setIsHomePageVisible] = useState(true); // Контрола на видливоста на почетната страница
    ...
    // Функција за скривање на почетната страница
    const handleViewDetailClick = () => {
        setIsHomePageVisible(false); // Скривање на почетната страница
        setIsCategoryVisible(false); // Скривање на категориите (ако се прикажани)
    };
    ...
    return (
        <div className="relative w-full flex flex-col items-start p-4">
            ...
            {/* Почетна страница - кога менито не е отворено и кога не е кликнато на "View Detail" */}
            {isHomePageVisible && !isCategoryVisible && (
                <div className="mt-4">
                    ...
                    {/* Копче за преглед на детали (View Detail) */}
                    <button
                        onClick={handleViewDetailClick} // Скривање на почетната страница и категориите
                        className="mt-4 px-4 py-2 bg-blue-500 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition-all"
                    >
                        View Detail
                    </button>
                </div>
            ...
            {isCategoryVisible && (
                <div className="relative w-full bg-white shadow-lg p-4 rounded-lg z-50" style={{ position: 'relative' }}>
                    <button
                        onClick={() => {
