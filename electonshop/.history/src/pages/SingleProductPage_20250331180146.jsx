    ...
    <div className="bg-white rounded-2xl shadow-xl flex flex-col md:flex-row overflow-hidden max-w-5xl mx-auto p-8 border border-gray-200">
        ...
        <div className="flex gap-3 mt-4">
          {product.images.map((el, index) => (
            <img
              ...
              ${index === currentImageIndex ? `border-[${colors.mainBlue}] scale-110` : 'border-gray-300 opacity-75'}`}
            />
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-col justify-between w-full md:w-1/2 pl-6">
        ...
        <div className="flex items-center mt-4">
          <button onClick={() => setCounter(prev => Math.max(1, prev - 1))} className="bg-[${colors.mainBlue}] text-white px-4 py-2 rounded-lg">-</button>
          <span className="mx-4 text-lg font-bold">{counter}</span>
          <button onClick={() => setCounter(prev => prev + 1)} className="bg-[${colors.mainBlue}] text-white px-4 py-2 rounded-lg">+</button>
        </div>
        
        {/* Копче за купување */}
        <button className="mt-6 bg-[${colors.mainYellow}] text-white px-6 py-3 rounded-lg shadow-lg hover:bg-yellow-500 transition flex items-center justify-center gap-2">
