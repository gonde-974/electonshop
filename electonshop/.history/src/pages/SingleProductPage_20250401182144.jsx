    ...
    <div className="max-w-6xl mx-auto p-6 bg-white shadow-lg rounded-2xl">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="md:w-1/2 space-y-4">
          ...
          <p className="text-gray-600">Категорија: <span className="font-medium text-blue-500">{product.category}</span></p>
          <p className="text-gray-600">Попуст: <span className="text-red-500">{product.discountPercentage}%</span></p>
          <p className="flex items-center text-gray-700">
            Оценка: {product.rating} <FaStar className="text-yellow-500 ml-1" />
          </p>
          <p className={`text-sm font-medium ${product.stock <= 5 ? 'text-red-500' : 'text-green-500'}`}>
            Залиха: {product.stock} парчиња
          </p>
          <p className="text-gray-700 text-sm">{product.description}</p>
          <p className="text-gray-600 font-medium">Бренд: {product.brand}</p>

          <hr className="my-4 border-gray-300" />

          {/* Бројач за количина */}
          <div className="flex items-center gap-4">
            <span className="text-gray-700 font-medium">Количина:</span>
            <button 
          ...
          <p className="text-lg font-semibold text-gray-800">
            Вкупно: <span className="text-blue-600">${(product.price * counter).toFixed(2)}</span>
          </p>
