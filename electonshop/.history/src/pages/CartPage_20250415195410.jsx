...
function CartPage() {
  ...
  const isCouponActive = curentCoupon === 'gonde';
          ...
          <div className='w-full lg:w-2/5 bg-lightBlueColor p-6 rounded-xl shadow-xl h-fit border border-gray-100'>
            ...
            {isCouponActive && (
              <div className='flex justify-between text-md font-medium mb-2 text-green-600'>
                <span>Попуст со купон:</span>
                <span>- ${ (totalPrice / 2).toFixed(2) }</span>
              </div>
            )}

            <div className='flex justify-between text-xl font-bold mb-6'>
              <span>За плаќање:</span>
            ...
            <div className='flex items-center gap-2 mb-6'>
              ...
              <button
                className={`px-4 py-2 ${
                  isCouponActive ? 'bg-red-500 hover:bg-red-600' : 'bg-mainYellow hover:bg-mainBlue'
                } text-white rounded-md transition`}
                onClick={handleCoupon}
              >
                {isCouponActive ? 'Отстрани' : 'Примени'}
              </button>
            </div>
