...
function SingleProductPage() {
  ...
  Залиха: <span className="font-bold">{product.stock}</span> парчиња
        ...
        {/* Бројач за количина */}
        <div className="flex items-center mt-4"><span className="mx-4 text-lg font-bold">Количина :</span>
          <button onClick={() => setCounter(prev => Math.max(1, prev - 1))} className="bg-gray-500 text-white px-3 py-2 rounded-lg">-</button>
          <span className="mx-4 text-lg font-bold">{counter}</span>
          <button onClick={() => setCounter(prev => prev + 1)} className="bg-gray-500 text-white px-3 py-2 rounded-lg">+</button>
        </div>
