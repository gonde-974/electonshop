...
function SingleProductPage() {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl flex flex-col md:flex-row overflow-hidden max-w-5xl mx-auto p-8 border border-gray-200">
      ...
      <div className="flex flex-col justify-between w-full md:w-1/2 pl-6">
        <div>
          <h2 className="text-3xl font-bold text-black mb-4">{product.title}</h2>
          <p className="text-2xl font-semibold text-mainRed mb-2">${product.price}</p>
          ...
          <p className={`text-sm font-semibold mb-2 ${product.stock <= 5 ? 'text-red-500' : 'text-gray-500'}`}>
  Залиха: <span className="font-bold">{product.stock}</span> парчиња
        ...
        {/* Бројач за количина */}
        <div className="flex items-center mt-4 gap-2 sm:gap-4">
  <span className="text-base sm:text-lg font-semibold text-gray-700">Количина:</span>

  <button 
    onClick={() => setCounter(prev => Math.max(1, prev - 1))} 
    className="bg-gray-300 text-gray-700 px-3 py-2 sm:px-4 sm:py-2 rounded-lg text-sm sm:text-base font-semibold shadow hover:bg-gray-400 transition"
  >
    -
  </button>

  <span className="text-lg sm:text-xl font-bold px-4 py-1 sm:px-5 sm:py-2 bg-gray-100 rounded-lg shadow-inner">
    {counter}
  </span>
